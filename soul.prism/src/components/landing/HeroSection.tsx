...
import { ArrowRight, Terminal, Send, GitBranch, Flame, Workflow, Activity, Shield } from "lucide-react";
...
const stats = [
    ...
    { value: "5", label: "Chaos Modes", detail: "Latency, Error, Drop, Load, Mock" },
        ...
        <section className="relative pt-24 pb-8 overflow-hidden">
            ...
            <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div className="text-center max-w-3xl mx-auto mb-10">
                    ...
                    >
                        <Button size="lg" className="font-semibold text-base px-8 h-12 glow-primary">
                        ...
                        <div className="grid md:grid-cols-[1fr_1px_1fr]">
                            ...
                            <div className="p-5 font-mono text-sm">
                                <div className="flex items-center gap-3 mb-4">
                                    <span className="px-2.5 py-1 rounded-md bg-primary/15 text-primary text-xs font-bold">GET</span>
                                ...
                                <div className="space-y-1.5 text-xs">
                                    <div className="flex items-center gap-2 text-muted-foreground/70">
                                        <span className="w-16 text-right text-muted-foreground/40">Header</span>
                                        <span className="text-primary/70">Authorization</span>
                                        <span className="text-muted-foreground/30">:</span>
                                        <span className="text-muted-foreground truncate">Bearer {"{{token}}"}</span>
                                    </div>
                                    <div className="flex items-center gap-2 text-muted-foreground/70">
                                        <span className="w-16 text-right text-muted-foreground/40">Header</span>
                                    ...
                                    <div className="flex items-center gap-2 text-muted-foreground/70">
                                        <span className="w-16 text-right text-muted-foreground/40">Env</span>
                                        <span className="text-primary/70">{"{{base_url}}"}</span>
                                        <span className="text-muted-foreground/30">=</span>
                                        <span className="text-muted-foreground truncate">api.example.com</span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="p-5 font-mono text-sm border-t md:border-t-0 border-border/50">
                                <div className="flex items-center justify-between mb-3">
                                    <span className="text-[10px] text-muted-foreground/50 uppercase tracking-wider">Response</span>
                                    <div className="flex items-center gap-2">
                                        <span className="text-xs px-2 py-0.5 rounded bg-primary/15 text-primary font-medium">200 OK</span>
                                        <span className="text-[10px] text-muted-foreground/40">432ms</span>
                                    </div>
                                </div>
                                <div className="rounded-lg bg-background/60 border border-border/30 p-3.5 text-xs leading-relaxed">
                                    ...
                                    <span className="text-muted-foreground/50">,</span><br />
                ...
                >
                    {stats.map((stat, i) => (
                        ...
                        >
                            ...
                            <div className="text-[10px] text-muted-foreground">{stat.detail}</div>
                        </motion.div>
                    ))}
                </motion.div>
            </div>
