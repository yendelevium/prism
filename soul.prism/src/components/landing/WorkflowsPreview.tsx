...
import { Play, CheckCircle2, XCircle, ArrowRight, CornerDownRight, SkipForward } from "lucide-react";

const steps = [
    { method: "POST", path: "/auth/login", status: "success", detail: "→ token", time: "89ms" },
    { method: "GET", path: "/api/user/profile", status: "success", detail: "→ user_id", time: "142ms" },
    { method: "PUT", path: "/api/user/settings", status: "success", detail: "", time: "283ms" },
    { method: "GET", path: "/api/notifications", status: "skipped", detail: "", time: "" },
    { method: "DELETE", path: "/api/session", status: "failed", detail: "", time: "502ms" },
];
                ...
                <div className="grid lg:grid-cols-[1fr_480px] gap-12 lg:gap-16 items-center">
                    ...
                    >
                        ...
                        <p className="text-muted-foreground leading-relaxed mb-8 max-w-lg">
                            Build multi-step test scenarios that pass data between requests, run assertions,
                            skip steps conditionally, and replay real user traces.
                        </p>
                        ...
                        <div className="relative rounded-2xl border border-border bg-card shadow-2xl overflow-hidden font-mono">
                            ...
                            <div className="p-4 border-b border-border bg-secondary/20 flex items-center justify-between">
                                ...
                                <div className="flex gap-2">
                                    <span className="text-[10px] px-2 py-0.5 rounded bg-secondary/50 text-muted-foreground">5 steps</span>
                                    <span className="text-[10px] px-2 py-0.5 rounded bg-destructive/15 text-destructive font-bold">1 failed</span>
                                </div>
                            </div>
                            ...
                            <div className="p-6 space-y-4 relative">
                                {steps.map((step, i) => (
                                    <div key={i} className="relative">
                                        ...
                                        <div className={`flex items-center gap-4 ${step.status === 'skipped' ? 'opacity-40' : ''}`}>
                                            <div className="relative z-10 flex items-center justify-center w-5 h-5 bg-card">
                                                {step.status === 'success' && <CheckCircle2 className="w-4 h-4 text-primary" />}
                                                {step.status === 'failed' && <XCircle className="w-4 h-4 text-destructive" />}
                                                {step.status === 'skipped' && <SkipForward className="w-4 h-4 text-muted-foreground" />}
                                            </div>

                                            <div className="flex-1 flex items-center justify-between text-[11px]">
                                                <div className="flex items-center gap-2">
                                                    <span className={`${step.status === 'failed' ? 'text-destructive' : 'text-muted-foreground'} font-bold`}>
                                                        {step.method}
                                                    </span>
                                                    <span className="text-foreground/80">{step.path}</span>
                                                </div>
                                                <div className="flex items-center gap-3 text-muted-foreground/50">
                                                    {step.detail && <span className="text-primary/60">{step.detail}</span>}
                                ...
                                <div className="absolute left-[9px] top-10 flex flex-col gap-[38px] opacity-20">
                                    {[1, 2, 3, 4].map(i => <CornerDownRight key={i} className="w-2.5 h-2.5 rotate-45" />)}
                                </div>
                                ...
                                >
                                    ...
                                    <div className="text-[10px] text-muted-foreground leading-normal font-mono">
                                        Expected: <span className="text-primary">200</span> • Got: <span className="text-destructive">500</span>
                                    </div>
                                </motion.div>
                            </div>
                        </div>
